...
http_listener::http_listener(boost::asio::io_context & io_context, boost::asio::ssl::context & secure_context, boost::asio::ip::tcp::endpoint & endpoint, std::shared_ptr<state> const & state) : _io_context(io_context), _acceptor(io_context), _socket(io_context), _secure_context(secure_context), _state(state) {
...
void http_listener::on_accept(boost::system::error_code error) {
    if ( error ) {
        ...
        std::make_shared<http_session> (std::move(_socket),_secure_context, _state)
                ->run();
    }
